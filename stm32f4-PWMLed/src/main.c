...
int main(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin =  PIN_LED_R|PIN_LED_B| PIN_LED_G|PIN_LED_O;
 ...
 /* Alternate functions*/
  GPIO_PinAFConfig(GPIOD, PIN_LED_O_Source, GPIO_AF_TIM4);
  GPIO_PinAFConfig(GPIOD, PIN_LED_R_Source, GPIO_AF_TIM4);
  GPIO_PinAFConfig(GPIOD, PIN_LED_G_Source, GPIO_AF_TIM4);
  GPIO_PinAFConfig(GPIOD, PIN_LED_B_Source, GPIO_AF_TIM4);
 ...
 TIM_TimeBaseStructure.TIM_Period = period;
  ...
  while(1){
    prescal--;
    Delay(50);
    TIM4PWM_SetDC(1,period*(0.5+(0.5*arm_cos_f32(x))));
    TIM4PWM_SetDC(2,period*(0.5+(0.5*arm_cos_f32(x+(PI/4)))));
    TIM4PWM_SetDC(3,period*(0.5+(0.5*arm_sin_f32(x))));
    TIM4PWM_SetDC(4,period*(0.5+(0.5*arm_sin_f32(x+(PI/4)))));
    if(!prescal){
